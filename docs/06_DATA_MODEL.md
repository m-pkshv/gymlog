...
### 5.2. `Equipment`
Аналогично: `barbell`, `dumbbell`, `kettlebell`, `machine`, `cable`, `bodyweight`, `pull_up_bar`, `dip_bars`, `band`, `cardio_machine`, `other`. Поля: `id`, `sortOrder`; названия в ARB.
`pull_up_bar`/`dip_bars` выделены из `other` в редизайне v2 (2026-08-05, решение владельца, при планировании иконок по оборудованию) — в `other` было ~49 разнородных упражнений, турник (11 шт.) и брусья (2 шт.) оказались достаточно крупными и визуально однозначными подгруппами, чтобы выделить их отдельно; остальное (канаты/сани/ролик для пресса/тумба и т.п.) осталось в `other`. Заодно исправлена реальная ошибка в данных: 17 упражнений с «cable» в названии/id (например `cable_crossover`, `high_cable_curl`) были ошибочно помечены как `machine` вместо `cable` — исправлено в том же обновлении сида.
...
| customIconPath | TEXT | нет | NULL | Путь к файлу на устройстве (не ассет) — иконка пользовательского упражнения в каталоге (S-06). Редизайн v2, 2026-08-04, запрос владельца |
| customImagePath | TEXT | нет | NULL | Путь к файлу на устройстве — большая картинка пользовательского упражнения на его карточке (S-07). Независима от customIconPath — это разные картинки |
...
| 9 | Редизайн v2, 2026-08-04 | Добавлены `Exercises.customIconPath`/`customImagePath` (раздел 6.1) — пользователь может загрузить свою иконку упражнения (каталог) и отдельную большую картинку (карточка упражнения), запрос владельца; `Migrator.addColumn` для обеих колонок (пустые для существующих строк); покрыто тем же тестом `database_migration_v1_to_current_test.dart` |
