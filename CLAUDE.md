...
- ⏭️ **Шаг 4** (следующий, не начат): домен-модели, enum'ы и интерфейсы репозиториев (D-13) + маппинг drift-строк в domain-модели.
- Дальше по плану Этапа 0 (порядок ориентировочный, может уточняться): DAO поверх `AppDatabase`; сиды справочников (MuscleGroup/Equipment/MeasurementType) и заглушечный сид 5 упражнений (DM 12); `go_router` с маршрутами (UX 4) + темы Material 3 из seed-цвета (UX 9) + каркас ARB RU/EN; логгер (TS 11.7, уровни debug/info/error, без `print`).